...
public class Calendar {
    public void getCalendar(int month) {
        ...
        DayOfWeek firstDayofOfWeek = DayOfWeek.from(LocalDate.of(year.getValue(), m.getValue(), 1));
        System.out.println("    CALENDAR FOR " + m + " " + year);
        System.out.println(" Mon Tue Wed Thu Fri" + (char) 27 + "[34m Sat Sun " + (char) 27 + "[37m");
        for (int i = 1; i < firstDayofOfWeek.getValue(); i++)
            System.out.print("    ");
        ...
        for (int i = 1; i <= lengthMonth; i++) {
            if ((m == LocalDate.now().getMonth()) && (i == LocalDate.now().getDayOfMonth())) {
                ...
                if (((i - 1 + firstDayofOfWeek.getValue()) % 7 == 0) || ((i + firstDayofOfWeek.getValue()) % 7 == 0))
                    System.out.print((char) 27 + "[34m");
                System.out.printf("%3d ", i);
            }
            if (((i - 1 + firstDayofOfWeek.getValue()) % 7 == 0) || (i == lengthMonth)) {
